...
(function () {
  function initialize({
    ...
    const { detectCompletedSegment } = window.BCLTypingSegmentDetector;
    const { requestTypingTranslation } = window.BCLTypingTranslationClient;
    ...
    function showStatus(text, className) {
      ...
      hint.className = "typing-translation-hint " + (className || "");
    ...
    function showItems(items) {
      ...
      items.forEach((item) => {
        if (!item || typeof item.translation !== "string") return;
    ...
    function isSourceLanguageTyping(text, profile) {
      if (!profile || profile.sourceLanguage !== "zh") {
        return false;
      }
    ...
    async function processSegment(segment) {
      if (!segment || consumed.has(segment.identity)) return;

      // Consume before request: REQUESTED is terminal for automatic triggering.
      ...
      if (!isSourceLanguageTyping(segment.text, profile)) {
        return;
      }

      showStatus("Typing Translation…", "is-loading");

      try {
        const items = await requestTypingTranslation({
          segment,
          sourceLanguage: profile.sourceLanguage,
          targetLanguage: profile.targetLanguage,
          levelSystem: profile.learningLevelSystem,
        ...
        showStatus(
          error && error.code
            ? "Typing Translation unavailable · " + error.code
            : "Typing Translation unavailable",
          "is-error"
        );
        hideTimer = setTimeout(clearHint, 5000);
      }
    }
    ...
    input.addEventListener("paste", () => {
      lastBeforeInput = { inputType: "insertFromPaste", data: "" };
    });

    input.addEventListener("drop", () => {
      lastBeforeInput = { inputType: "insertFromDrop", data: "" };
    });
    ...
    input.addEventListener("input", () => {
      ...
      if (!eventInfo || eventInfo.inputType !== "insertText") {
        return;
      }
    ...
    document.addEventListener(
      ...
      (event) => {
        if (!event.detail || event.detail.enabled !== true) {
          clearHint();
        }
      }
    );
